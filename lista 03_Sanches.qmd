...
a) SELECT DNO, SALARY FROM EMPLOYEE
Apenas mostra todos as informações relacionadas as colunas escolhidas sem nenhuma order em especifíco e também repete alguns valores
```{sql connection=con}
SELECT DNO, SALARY FROM EMPLOYEE
```
b) SELECT DISTINCT DNO, SALARY FROM EMPLOYEE
Resulta na mesma tabela do item a, apesar do uso do "DISTINCT", ambas as colunas não compartilham o mesmo DNO e SALARY simultaneamente
```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE
```
c) SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
Nenhuma linha foi removida, porém o DNO foi ordenado em ordem crescente, tendo a ordem da coluna DNO como prioridade e a de SALARY apenas seguindo a ordem proposta por DNO
...
```{sql connection=con}
SELECT PNO, PNAME, STATE, FEE, ACRES FROM PRESERVE
...
```{sql connection=con}
SELECT PNO, PNAME, STATE, ACRES, FEE FROM PRESERVE
...
```{sql connection=con}
SELECT PNO, FEE, PNAME FROM PRESERVE
WHERE FEE > 0
